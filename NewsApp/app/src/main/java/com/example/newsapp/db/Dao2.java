...
public class Dao2 {
    ...
    public int Insert(NewsHeadLine newsHeadLine){
        ...
        long kq = sqLiteDatabase.insert("data2" , null, values);
        if(kq==0)
            return -1;
        return 1;
    ...
    public List<NewsHeadLine> getAll(){
        ...
        Cursor cursor = sqLiteDatabase.rawQuery("Select * from data2", null);
...
//                ,cursor.getString(3),cursor.getString(4),cursor.getString(5),cursor.getString(6),cursor.getString(7)));
    ...
    public int Del(String tt){
        SQLiteDatabase sqLiteDatabase= dbhp.getWritableDatabase();
        long kq= sqLiteDatabase.delete("data2", "title =? ", new String[]{tt});
        if(kq==0)
            return -1;
        return 1;
    }
